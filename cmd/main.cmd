...
@IF /I "TRUE" EQU "%SOLOMON_PREVIOUS_COMPILE_WARNED_BUILD_FIRST%" SET SOLOMON_BUILD_LIST=%SOLOMON_PREVIOUS_COMPILE_WARNED_BUILD% %SOLOMON_BUILD_LIST%
...
@IF /I "TRUE" EQU "%SOLOMON_PREVIOUS_COMPILE_FAILED_BUILD_FIRST%" SET SOLOMON_BUILD_LIST=%SOLOMON_PREVIOUS_COMPILE_FAILED_BUILD% %SOLOMON_BUILD_LIST%
...
@REM
@REM masterからtestshotへミラー
@REM

@ECHO %SOLOMON_MESSAGE_PREFIX%スナップショット作成(master to testshot)

@CALL "%SOLOMON_MIRROR_DIR_CMD%" "%SOLOMON_SNAPSHOT_MASTER_DIR%" "%SOLOMON_TESTSHOT_DIR%"
...
@REM ビルドコマンドの実行
@PUSHD "%SOLOMON_TESTSHOT_DIR%"
@IF NOT EXIST "%SOLOMON_TESTSHOT_DIR%\%SOLOMON_BUILD_CMD_DIR%\%SOLOMON_BUILD_HEAD%.cmd" COLOR 0C&ECHO %SOLOMON_MESSAGE_PREFIX%"%SOLOMON_TESTSHOT_DIR%\%SOLOMON_BUILD_CMD_DIR%\%SOLOMON_BUILD_HEAD%.cmd" が存在しません。設定を確認してください。&GOTO :EOF
@CALL "%SOLOMON_TESTSHOT_DIR%\%SOLOMON_BUILD_CMD_DIR%\%SOLOMON_BUILD_HEAD%.cmd"
@POPD
